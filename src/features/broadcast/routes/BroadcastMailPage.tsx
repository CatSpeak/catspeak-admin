...
const BroadcastMailPage: React.FC = () => {
  ...
  // Targets
  const [targetType, setTargetType] = useState<"all" | "roles" | "countries" | "custom">("all");
  ...
  const handleEstimate = async () => {
    ...
    try {
      ...
      const res = await axiosClient.post<{ estimatedCount: number }>("/Admin/broadcasts/estimate", {
  ...
  }, [targetType, selectedRoles, selectedCountries, importedEmailsStr, activeTab]);

  // Fetch Broadcast History
  const fetchHistory = async () => {
    setLoadingHistory(true);
    try {
      const res = await axiosClient.get<{ items: BroadcastItem[] }>("/Admin/broadcasts?page=1&pageSize=50");
  ...
  const handleCsvFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await axiosClient.post<{ validEmails: string[]; totalFound: number }>("/Admin/broadcasts/parse-csv", formData, {
      ...
      if (parsedEmails.length > 0) {
        ...
        alert(`Đã trích xuất thành công ${parsedEmails.length} email từ tệp CSV/Excel!`);
      } else {
        alert("Không tìm thấy email hợp lệ trong tệp.");
      }
    } catch (err: any) {
      alert("Lỗi khi đọc file CSV: " + (err.response?.data?.message || err.message));
    } finally {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) {
      setMessage({ type: "error", text: "Vui lòng nhập tên chiến dịch broadcast." });
      return;
    }
    if (!subjectVi.trim() || !contentVi.trim()) {
      setMessage({ type: "error", text: "Vui lòng nhập Tiêu đề & Nội dung Tiếng Việt (bắt buộc)." });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: "success", text: "Chiến dịch email broadcast đã được khởi tạo thành công!" });
      setActiveTab("history");
      fetchHistory();
    } catch (err: any) {
      setMessage({ type: "error", text: err.response?.data?.message || "Không thể tạo chiến dịch broadcast." });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleCancelCampaign = async (id: number) => {
    if (!window.confirm(`Bạn có chắc chắn muốn HỦY chiến dịch Broadcast #${id}? Worker ngầm sẽ dừng gửi ngay lập tức.`)) return;

    try {
      await axiosClient.post(`/Admin/broadcasts/${id}/cancel`);
      alert(`Đã hủy thành công chiến dịch #${id}.`);
      fetchHistory();
    } catch (err: any) {
      alert("Lỗi khi hủy chiến dịch: " + (err.response?.data?.message || err.message));
    }
  };

  // Delete Campaign
  const handleDeleteCampaign = async (id: number) => {
    if (!window.confirm(`Bạn có chắc chắn muốn XÓA chiến dịch Broadcast #${id} và toàn bộ nhật ký liên quan?`)) return;

    try {
      await axiosClient.delete(`/Admin/broadcasts/${id}`);
      fetchHistory();
    } catch (err: any) {
      alert("Lỗi khi xóa chiến dịch: " + (err.response?.data?.message || err.message));
    }
  };
  ...
  const fetchBroadcastLogs = async (broadcastId: number, page: number, statusFilter: string, search: string) => {
    setLoadingLogs(true);
    try {
      ...
      const res = await axiosClient.get<{ items: BroadcastLogItem[]; totalPages: number }>(url);
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      ...
      alert(`Đã gửi email thử nghiệm tới ${testEmail} thành công!`);
      setTestModalOpen(false);
    } catch (err: any) {
      alert("Lỗi khi gửi email thử nghiệm: " + (err.response?.data?.message || err.message));
    } finally {
  ...
  const getActiveContentHtml = () => {
    ...
    return body || "<p style='color: #888; text-align: center;'>Chưa có nội dung để xem trước</p>";
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-[#8f0d15]/10 text-[#8f0d15] rounded-xl">
            <Mail className="w-7 h-7" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Gửi Mail Hàng Loạt (Broadcast Email)</h1>
            <p className="text-sm text-gray-500">
              Hệ thống gửi tin tức, sự kiện & thông báo tới hàng ngàn người dùng CatSpeak theo batch ngầm (Rate-limited Queue).
            </p>
          </div>
        </div>
      ...
      {activeTab === "compose" ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Tên Chiến Dịch Broadcast</label>
                <input
                  type="text"
                  placeholder="Ví dụ: Thông Báo Cập Nhật CatSpeak V2.5 & Tính Năng Mới"
                  value={title}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                <div>
                  <span className="text-sm font-semibold text-gray-900 block">Smart Multi-Lingual Variants</span>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-3">
                <div className="flex items-center gap-2">
                  {isMultiLanguage ? (
                    <>
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg ${
                          langTab === "vi" ? "bg-[#8f0d15] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                        }`}
                      >
                        🇻🇳 Tiếng Việt
                      </button>
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg ${
                          langTab === "en" ? "bg-[#8f0d15] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                        }`}
                      >
                        🇬🇧 Tiếng Anh
                      </button>
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg ${
                          langTab === "zh" ? "bg-[#8f0d15] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                        }`}
                      >
                        🇨🇳 Tiếng Trung
                      </button>
                    ...
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Mẫu Email Duy Nhất</span>
                  )}
                </div>
                ...
                <div className="flex items-center bg-gray-100 p-1 rounded-lg">
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs font-semibold rounded ${
                      viewMode === "code" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500"
                    } flex items-center gap-1`}
                  >
                    <Code className="w-3.5 h-3.5" /> Mã HTML
                  </button>
                  <button
              ...
              <div className="flex items-center gap-2 text-xs bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                <span className="font-semibold text-gray-600">Chèn Biến Thể:</span>
                <button
                ...
                <>
                  ...
                  {(langTab === "vi" || !isMultiLanguage) && (
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Tiêu Đề Email (Subject - VI)</label>
                        <input
                          type="text"
                          placeholder="[CatSpeak] Thông báo cập nhật hệ thống quan trọng..."
                          value={subjectVi}
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Nội Dung Email (HTML Code - VI)</label>
                        <textarea
                          rows={10}
                          placeholder="<h2>Xin chào {{name}},</h2><p>Nội dung thông báo gửi tới người dùng...</p>"
                          value={contentVi}
                  ...
                  {isMultiLanguage && langTab === "en" && (
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Subject Line (EN)</label>
                        <input
                          type="text"
                          placeholder="[CatSpeak] System Update & New Features Announcement..."
                          value={subjectEn}
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Email Body (HTML Code - EN)</label>
                        <textarea
                          rows={10}
                          placeholder="<h2>Hello {{name}},</h2><p>We are excited to announce...</p>"
                          value={contentEn}
                  ...
                  {isMultiLanguage && langTab === "zh" && (
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">邮件主题 (ZH)</label>
                        <input
                          type="text"
                          placeholder="[CatSpeak] 系统更新与新功能通知..."
                          value={subjectZh}
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">邮件内容 (HTML Code - ZH)</label>
                        <textarea
                          rows={10}
                          placeholder="<h2>尊敬的用户 您好：</h2><p>我们很荣幸向您宣布...</p>"
                          value={contentZh}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
              <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
                <Users className="w-5 h-5 text-[#8f0d15]" /> Đối Tượng Nhận Mail
              </h3>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-medium text-gray-800">Tất cả người dùng (All Active Users)</span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="text-[#8f0d15] focus:ring-[#8f0d15]"
                  />
                  <span className="text-sm font-medium text-gray-800">Lọc Theo Vai Trò (Roles)</span>
                </label>
                  ...
                  <div className="pl-6 space-y-1 text-xs">
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedRoles(
                            e.target.checked ? [...selectedRoles, 1] : selectedRoles.filter((r) => r !== 1)
                          )
                        }
                      />
                      Học viên / User Thường (Role ID 1)
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        ...
                        onChange={(e) =>
                          setSelectedRoles(
                            e.target.checked ? [...selectedRoles, 2] : selectedRoles.filter((r) => r !== 2)
                          )
                        }
                      />
                      Giảng viên / Instructor (Role ID 2)
                    </label>
                  </div>
                )}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="text-[#8f0d15] focus:ring-[#8f0d15]"
                  />
                  <span className="text-sm font-medium text-gray-800">Lọc Theo Quốc Gia (Country)</span>
                </label>

                {targetType === "countries" && (
                  <div className="pl-6 space-y-1 text-xs">
                    {["Vietnam", "China", "United States", "Japan", "Korea"].map((c) => (
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setSelectedCountries(
                              e.target.checked ? [...selectedCountries, c] : selectedCountries.filter((item) => item !== c)
                            )
                          }
                        />
                        {c}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="text-[#8f0d15] focus:ring-[#8f0d15]"
                  />
                  <span className="text-sm font-medium text-gray-800">Import Danh Sách Email (CSV / Text)</span>
                </label>
                ...
                {targetType === "custom" && (
                  <div className="pl-6 pt-1 space-y-2">
                    ...
                    >
                      ...
                      Tải Tệp CSV / Excel Lên
                    </button>

                    <textarea
                      rows={4}
                      placeholder="Mỗi email một dòng hoặc phân cách bởi dấu phẩy..."
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-200/60 flex items-center justify-between">
                <div>
                  <span className="text-xs text-gray-500 block">Dự kiến số mail sẽ gửi:</span>
                  <span className="text-xl font-extrabold text-[#8f0d15]">
                    {estimating ? "..." : estimatedRecipients !== null ? `${estimatedRecipients.toLocaleString()} người nhận` : "N/A"}
                  </span>
                </div>
                <button
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
              <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
                <Clock className="w-5 h-5 text-[#8f0d15]" /> Lập Lịch & Gửi
              </h3>
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="text-[#8f0d15] focus:ring-[#8f0d15]"
                  />
                  <span className="text-sm font-medium text-gray-800">Lập lịch gửi sau</span>
                </label>
              ...
              <div className="pt-3 border-t border-gray-100 space-y-2">
                ...
                >
                  <Sparkles className="w-4 h-4 text-[#8f0d15]" /> Gửi Email Thử Nghiệm (Test)
                </button>
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      <Send className="w-4 h-4" /> {isScheduled ? "Xác Nhận Lập Lịch Gửi" : "Bắt Đầu Gửi Hàng Loạt Right Now"}
                    </>
                  )}
                </button>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-gray-900">Lịch Sử & Tiến Độ Chiến Dịch Email Broadcast</h2>
              <p className="text-xs text-gray-500">Theo dõi trạng thái gửi mail theo real-time background worker</p>
            </div>
            <button
            ...
            >
              <RefreshCw className={`w-3.5 h-3.5 ${loadingHistory ? "animate-spin" : ""}`} /> Làm mới
            </button>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50 text-gray-600 font-semibold text-xs border-b border-gray-100">
                <tr>
                  ...
                  <th className="p-4">Tiến Độ Gửi (%)</th>
                  <th className="p-4">Thành Công / Thất Bại</th>
                ...
                {historyItems.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="p-8 text-center text-gray-400">
                      Chưa có chiến dịch broadcast nào được thực hiện.
                    </td>
                  </tr>
                ) : (
                  historyItems.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/60 transition-all">
                      <td className="p-4 font-mono font-bold text-xs text-gray-500">#{item.id}</td>
                      <td className="p-4 font-semibold text-gray-900">
                        {item.title}
                        <span className="block text-xs font-normal text-gray-500 truncate max-w-xs">{item.subjectVi}</span>
                      </td>
                      <td className="p-4">
                        {item.isMultiLanguage ? (
                          <span className="px-2.5 py-0.5 text-[11px] font-bold bg-purple-100 text-purple-700 rounded-full">
                            Multi-Lingual (VI/EN/ZH)
                          </span>
                        ) : (
                      ...
                      <td className="p-4">
                        <div className="w-36 space-y-1">
                          <div className="flex justify-between text-xs font-semibold">
                            ...
                            <span className="text-gray-400">{item.sentCount}/{item.totalRecipients}</span>
                          </div>
                      ...
                      <td className="p-4 text-xs font-medium">
                        <span className="text-green-600 font-bold">{item.sentCount} OK</span> /{" "}
                        <span className="text-red-500 font-bold">{item.failedCount} Fail</span>
                      </td>
                      <td className="p-4 text-xs text-gray-500">{new Date(item.createdAt).toLocaleString()}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-4xl w-full space-y-4 shadow-xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div>
                <h3 className="text-lg font-bold text-gray-900">
                  Nhật Ký Chi Tiết Campaign #{selectedBroadcast.id}: {selectedBroadcast.title}
                </h3>
                <p className="text-xs text-gray-500">
                  Tổng {selectedBroadcast.totalRecipients} người nhận | Thành công: {selectedBroadcast.sentCount} | Thất bại: {selectedBroadcast.failedCount}
                </p>
              </div>
              <button onClick={() => setLogModalOpen(false)} className="text-gray-400 hover:text-gray-600 p-1">
                ✕
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    fetchBroadcastLogs(selectedBroadcast.id, 1, e.target.value, logSearch);
                  }}
                  className="px-2.5 py-1.5 border border-gray-200 rounded-lg focus:outline-none"
                >
                  <option value="all">Tất cả (All)</option>
                  <option value="1">Thành công (Sent)</option>
                  <option value="2">Thất bại (Failed)</option>
                  <option value="0">Đang chờ (Pending)</option>
                </select>
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") fetchBroadcastLogs(selectedBroadcast.id, 1, logStatusFilter, logSearch);
                  }}
                  className="px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none w-48"
                />
                <button
                  onClick={() => fetchBroadcastLogs(selectedBroadcast.id, 1, logStatusFilter, logSearch)}
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-gray-50 text-gray-600 font-semibold sticky top-0 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="p-3">Chi Tiết Lỗi (nếu có)</th>
                  ...
                  {loadingLogs ? (
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-gray-400">
                        <Loader2 className="w-5 h-5 animate-spin mx-auto mb-2" /> Đang tải nhật ký...
                      </td>
                    </tr>
                  ) : logsList.length === 0 ? (
                    <tr>
                    ...
                    logsList.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50/60">
                        <td className="p-3 font-semibold text-gray-900">{log.recipientEmail}</td>
                        <td className="p-3 uppercase font-mono font-bold">{log.languageUsed}</td>
                        <td className="p-3">
                          <span
                        ...
                        <td className="p-3 text-red-600 font-mono text-[11px] truncate max-w-xs">{log.errorMessage || "-"}</td>
                        <td className="p-3 text-gray-500">{log.sentAt ? new Date(log.sentAt).toLocaleString() : "-"}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            <div className="flex items-center justify-between text-xs text-gray-500 pt-2 border-t border-gray-100">
              <span>Trang {logPage} / {logTotalPages}</span>
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => {
                    const p = logPage - 1;
                    setLogPage(p);
                    fetchBroadcastLogs(selectedBroadcast.id, p, logStatusFilter, logSearch);
                  }}
      ...
      {testModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 shadow-xl">
            <h3 className="text-lg font-bold text-gray-900">Gửi Email Thử Nghiệm (Test Preview)</h3>
            <p className="text-xs text-gray-500">
              Nhập email cá nhân của bạn để kiểm tra hiển thị mail thật trong hộp thư trước khi thực hiện phát sóng hàng loạt.
            </p>
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Email Nhận Thử Nghiệm</label>
              <input
                type="email"
