...
import { Plus, CheckCircle2, Trash2, ShieldAlert, Edit, RotateCcw } from "lucide-react";
...
import { getScripts, deleteScripts, updateScriptsStatus } from "../api/scriptApi";
...
export default function ScriptsPage() {
  ...
  const fetcher = useCallback(async (page: number, pageSize: number) => {
    try {
      ...
      setSelectedIds([]);
      return { data: res.data, total: res.additionalData?.totalCount ?? res.totalRecords ?? 0 };
    } catch (err) {
      addToast("error", t.scripts?.loadError || "Không thể tải danh sách script.");
      return { data: [], total: 0 };
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      addToast("success", (t.scripts?.deleteSuccess || "Đã xóa {count} script.").replace("{count}", deleteIds.length.toString()));
      setDeleteIds([]);
  ...
  const handleBulkStatus = async (newStatus: ScriptStatus, ids: number[] = selectedIds) => {
    if (ids.length === 0) return;
    try {
      await updateScriptsStatus(ids, newStatus);
      addToast("success", (t.scripts?.statusUpdateSuccess || "Đã cập nhật trạng thái {count} script.").replace("{count}", ids.length.toString()));
      setRefreshKey(prev => prev + 1);
      setSelectedIds([]);
    } catch (err) {
      addToast("error", t.scripts?.statusUpdateError || "Không thể cập nhật trạng thái.");
    }
  };
  ...
  const headers = useMemo(() => [
                  ...
                  <div className="flex flex-wrap gap-1">
                    {s.communities && s.communities.length > 0 ? (
                      <div className="flex flex-col gap-1.5">
                        <FlagBadge languageType={s.communities[0]} variant="default" showLabel={true} />
                ...
                render: (s: Script) => (
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm text-primary font-semibold bg-red-50 px-2 py-1 rounded-md w-fit whitespace-nowrap">
                      {s.highlights?.length || 0} {t.scripts?.highlightsCount ? t.scripts?.highlightsCount.replace("{count}", "") : "từ khóa"}
                    </span>
                  </div>
                )
              },
              {
                ...
                render: (s: Script) => (
                  s.status === "Published" ? (
                    <Badge type="Green" title={t.scripts?.badgePublished || "● Xuất bản"} />
                  ) : (
              ...
              },
              {
                ...
                render: (s: Script) => <span className="whitespace-nowrap">{formatDateTime(s.createdAt)}</span>
              },
              {
                ...
                render: (s: Script) => {
                  return <ScriptActionsCell s={s} navigate={navigate} handleBulkStatus={handleBulkStatus} setDeleteIds={setDeleteIds} />;
                }
              }
  ...
  return (
    <div className="space-y-6">
      <Breadcrumb items={[{ label: "Cat Speak", href: "/" }, { label: t.scripts?.breadcrumb || "Script" }]} />

      <PageHeader
        icon={<ShieldAlert />}
        title={t.scripts?.title || "Script tương tác"}
        desc={t.scripts?.desc || "Quản lý bài đọc tương tác, từ khóa học thuật và tài liệu luyện đọc cho học viên trên toàn hệ thống"}
        rightButtons={[
          <Button key="create" variant="primary" onClick={() => navigate("/scripts/create")}>
      ...
      {selectedIds.length > 0 && (
        <div className="bg-[#fdf2f2] border border-red-100 rounded-xl p-4 flex flex-col gap-4 animate-[fadeIn_150ms_ease-out]">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-gray-900">{(t.scripts?.selectedCount || "Đã chọn {count} script tương tác").replace("{count}", selectedIds.length.toString())}</span>
            <span className="text-gray-300 font-light text-lg">|</span>
            <span className="text-gray-500 text-sm">{t.scripts?.applyToAll || "Áp dụng hành động cho tất cả mục đã đánh dấu:"}</span>
          </div>
          <div className="flex items-center gap-3">
            <Button
              size="sm"
            ...
            >
              <CheckCircle2 className="w-4 h-4 mr-2" /> {t.scripts?.publishBulk || "Xuất bản hàng loạt"}
            </Button>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              <Edit className="w-4 h-4 mr-2" /> {t.scripts?.draftBulk || "Chuyển về Nháp"}
            </Button>
            <Button
              size="sm"
              variant="outline"
        ...
        <div className="flex flex-col sm:flex-row gap-3 p-4 bg-white border-b border-gray-200">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={t.scripts?.searchPlaceholder || "Tìm theo tiêu đề script hoặc từ khóa..."}
              value={keyword}
          ...
          <select
            className="block w-full sm:w-48 pl-3 pr-10 py-2 text-base border-gray-700 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-lg"
            value={community}
            onChange={(e) => {
          ...
          >
            <option value="All">{t.scripts?.communityAll || "Cộng đồng: Tất cả"}</option>
          ...
          <select
            className="block w-full sm:w-48 pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-lg"
            value={status}
            onChange={(e) => {
          ...
          >
            <option value="All">{t.scripts?.statusAll || "Trạng thái: Tất cả"}</option>
            <option value="Published">{t.scripts?.published || "Đã xuất bản"}</option>
            <option value="Draft">{t.scripts?.draft || "Bản nháp"}</option>
          </select>

          <Button variant="outline" onClick={resetFilters} className="rounded-lg px-3 hover:bg-gray-50 transition-colors">
